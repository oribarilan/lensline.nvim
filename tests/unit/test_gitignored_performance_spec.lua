...
describe("gitignored files per-file checking", function()
  ...
  describe("edge cases", function()
    it("should handle files outside git repository", function()
      local limits = require("lensline.limits")
      limits.clear_cache()
      ...
      local bufnr = vim.api.nvim_create_buf(false, true)
      vim.api.nvim_buf_set_name(bufnr, "/tmp/non_git_file.lua")
      vim.api.nvim_buf_set_lines(bufnr, 0, -1, false, { "line 1" })

      local should_skip = limits.should_skip(bufnr)

      assert.is_false(system_called, "Should not call git when no .git directory found")
      assert.is_false(should_skip, "Should not skip files outside git repo")

      vim.api.nvim_buf_delete(bufnr, { force = true })
    end)

    it("should cache false result for non-git files", function()
      local limits = require("lensline.limits")
      limits.clear_cache()
      ...
      local finddir_calls = 0
      local original_finddir_impl = vim.fn.finddir
      vim.fn.finddir = function(pattern, path)
        if pattern == ".git" then
          finddir_calls = finddir_calls + 1
          return ""
        end
        return original_finddir_impl(pattern, path)
      end
      ...
      local bufnr = vim.api.nvim_create_buf(false, true)
      vim.api.nvim_buf_set_name(bufnr, "/tmp/non_git_cache.lua")
      vim.api.nvim_buf_set_lines(bufnr, 0, -1, false, { "line 1" })
      ...
      assert.equals(1, finddir_calls, "Should cache non-git result")

      vim.api.nvim_buf_delete(bufnr, { force = true })
    end)

    it("should handle nested directory structure correctly", function()
      local limits = require("lensline.limits")
      limits.clear_cache()
      ...
      vim.fn.system = function(cmd)
        if type(cmd) == "table" and cmd[1] == "git" and cmd[4] == "check-ignore" then
          table.insert(checked_paths, cmd[6])
        end
        return ""
      end
      ...
      local bufnr = vim.api.nvim_create_buf(false, true)
      vim.api.nvim_buf_set_name(bufnr, "/test/project/src/nested/deep/file.lua")
      vim.api.nvim_buf_set_lines(bufnr, 0, -1, false, { "line 1" })

      limits.should_skip(bufnr)

      assert.equals(1, #checked_paths, "Should check the file path")

      vim.api.nvim_buf_delete(bufnr, { force = true })
    end)

    it("should clear gitignore cache on clear_cache call", function()
      local limits = require("lensline.limits")
      limits.clear_cache()
      ...
      local bufnr = vim.api.nvim_create_buf(false, true)
      vim.api.nvim_buf_set_name(bufnr, "/test/project/clear_cache_test.lua")
      vim.api.nvim_buf_set_lines(bufnr, 0, -1, false, { "line 1" })

      limits.should_skip(bufnr)
      assert.equals(1, check_count, "First check should call git")

      limits.should_skip(bufnr)
      assert.equals(1, check_count, "Cached result should not call git")

      limits.clear_cache()

      limits.should_skip(bufnr)
      assert.equals(2, check_count, "After clear_cache should call git again")

      vim.api.nvim_buf_delete(bufnr, { force = true })
    end)

    it("should handle multiple different files concurrently", function()
      local limits = require("lensline.limits")
      limits.clear_cache()
      ...
      vim.fn.system = function(cmd)
        if type(cmd) == "table" and cmd[1] == "git" and cmd[4] == "check-ignore" then
          table.insert(checked_files, cmd[6])
        end
        return ""
      end
      ...
      for i = 1, 5 do
        local bufnr = vim.api.nvim_create_buf(false, true)
        vim.api.nvim_buf_set_name(bufnr, "/test/project/concurrent_" .. i .. ".lua")
        vim.api.nvim_buf_set_lines(bufnr, 0, -1, false, { "line 1" })
        table.insert(buffers, bufnr)
        limits.should_skip(bufnr)
      end

      assert.equals(5, #checked_files, "Should check all 5 different files")
